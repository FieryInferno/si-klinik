...
@section('content')
  <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    ...
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      <a class="btn btn-primary shadow-md mr-2" href="{{ url('obat/create') }}">Tambah</a>
    </div>
  </div>
        ...
        <tbody>
          ...
          @foreach ($data as $dataTable)
            <tr>
              ...
              <td class="whitespace-nowrap">{{ $dataTable->kode }}</td>
              <td class="whitespace-nowrap">{{ $dataTable->nama }}</td>
              <td class="whitespace-nowrap">{{ $dataTable->harga }}</td>
              <td class="whitespace-nowrap">
                <a href="{{ url('obat/' . $dataTable->id . '/edit') }}" class="btn btn-success">Edit</a>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-body p-10 text-center">
            ...
            <form action="{{ url('obat/' . $dataModal->id) }}" method="post" class="text-right">
              @csrf
              {{ method_field('DELETE') }}
